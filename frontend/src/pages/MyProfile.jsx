...
const MyProfile = () => {
  ...
  const updateUserProfileData = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        setIsEdit(false);
        setImage(false); // reset preview
        loadUserProfileData(); // refresh from backend
      } else {
  ...
  return userData && (
    <div className='w-full flex flex-col items-start gap-0.5 text-sm'>
      {/* Profile Image */}
      {isEdit ? (
        <label htmlFor='image'>
      ...
      <hr className='bg-zinc-400 h-[1px] border-none mt-2 mb-2 w-1/4' />
